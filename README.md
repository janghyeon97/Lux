...
**공통 베이스**
`ULuxActionTask`는 모든 태스크의 기본 클래스로, 생명주기 관리와 Re-Home 지원을 담당합니다.

**애니메이션**
`LuxActionTask_PlayMontageAndWait`는 애니메이션 몽타주를 재생하고 노티파이를 태그 이벤트로 게시합니다.

**이동**
`LuxActionTask_LeapToLocation`은 곡선 도약, `LuxActionTask_FollowSpline`은 스플라인 추종, `LuxActionTask_LandingControl`은 착지 제어를 담당합니다.

**흐름/입력**
지연, 페이즈 대기, 이벤트 대기, 입력 대기, 서버 대기 등의 태스크로 액션의 흐름을 제어합니다.

**Aurora 특화**
`LuxActionTask_TraceAndBuildPath`는 지형 트레이스 기반 경로 샘플링/스무딩을 통해 스플라인 포인트를 생성합니다.

#### **샘플 액션 구현 포인트**

**`UAuroraAction_Hoarfrost`**
서버에서 `AHoarfrost` 액터를 스폰하고, 클라이언트에서는 `Cue_Aurora_Hoarfrost_FreezeSegments` 큐를 실행하여 장판 동결 효과를 구현합니다.

**`UAuroraAction_GlacialCharge`**
Begin에서 경로 분석 후 Dash에서 스플라인 추종과 얼음 방패 큐를 조합하여 돌진 능력을 구현합니다.

**`UAuroraAction_FrozenSimulacrum`**
속도와 조준 방향을 기반으로 8방향 섹션을 선택하여 분신을 생성하고 독립적인 AI로 동작시킵니다.

**`UAuroraAction_FrozenSword`**
클라이언트 예측과 서버 권위를 조합하여 `Task_Event_Attack_HitPredicted` → `Phase_ProcessHit` 순서로 데미지를 처리합니다.

**`UAuroraAction_Cryoseism`**
Leap/Impact/Landing/Recovery 페이즈를 거치며 중력 제어와 `ACryoseismExplosion` 체인 폭발을 구현합니다.
...
**UI 연동**: `UW_ActionIcon`이 델리게이트로 원형 게이지를 실시간 업데이트합니다.
...
**Cue 실행/정지/풀링**: 모든 큐의 생명주기를 중앙에서 관리하며, 풀링 시스템으로 메모리 효율성을 높입니다.
**액터별/태그별 큐 카운트**: 중복 실행 방지와 우선순위 조정을 위해 활성 큐를 추적합니다.

#### **Aurora 샘플 Cue**

**Cue_Aurora_*** 태그 그룹으로 Aurora 캐릭터의 모든 능력에 대한 시각적 피드백을 제공합니다.

**Hoarfrost_FreezeSegments**: 장판 동결 효과, **GlacialCharge_FrostShield**: 얼음 방패 효과, **FrozenSword_Primary**: 기본 공격 효과, **Ultimate_***: 궁극기 관련 효과를 담당합니다.
...
게임 데이터/큐/이펙트/CC 데이터를 사전 로드하고 캐시하여 성능을 최적화하는 시스템입니다.

**태그 스택**: `FGameplayTagStackContainer`
게임플레이 태그를 스택 형태로 관리하며, 상태 효과, 차단 효과, 콤보 카운터 등의 중첩을 추적합니다.

**로그 채널**: `LogLux*` (`LuxLogChannels.*`)
`LogLux`, `LogLuxActionSystem`, `LogLuxCooldown`, `LogLuxCombat` 등 시스템별 로그를 분류하여 디버깅과 모니터링을 지원합니다.
